...
> **Portability note.** This skill builds assets in the **Codex pet format**
> (an 8x9 atlas + `pet.json`/`spritesheet.webp` for the Codex app) and relies
> on an image-generation skill referred to below as `imagegen`. Claude Code
> has no built-in pet feature or `imagegen` skill, so this skill is included
> for portability: wherever it says `imagegen`, substitute whatever
> image-generation capability is available, and treat the output format as a
> Codex artifact. The deterministic packaging scripts work unchanged.
...
```text
${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/.system/imagegen/SKILL.md
```
...
```bash
SKILL_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/hatch-pet"
...
For the built-in path, record the selected source image from `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/generated_images/.../ig_*.png`. Do not record files from the run directory, `tmp/`, hand-made fixtures, deterministic row folders, or post-processed copies as visual job sources.
...
Package output is written outside the run directory by default. If `CLAUDE_CONFIG_DIR` is set, use it; otherwise use `$HOME/.claude`.

```text
${CLAUDE_CONFIG_DIR:-$HOME/.claude}/pets/<pet-name>/
  pet.json
  spritesheet.webp
```
...
8. Each subagent receives the row prompt and every listed input image path, invokes `imagegen`, and returns only the selected `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/generated_images/.../ig_*.png` source path.
...
- Tell the subagent to return only the selected original `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/generated_images/.../ig_*.png` source path plus a one-sentence QA note. The parent decides whether to record or repair it.
...
selected_source=/absolute/path/to/${CLAUDE_CONFIG_DIR:-$HOME/.claude}/generated_images/.../ig_*.png
...
- `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/pets/<pet-name>/pet.json` and `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/pets/<pet-name>/spritesheet.webp` are staged together for custom pets.
