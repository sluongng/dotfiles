...
```bash
bash <path-to-skill>/scripts/ensure_macos_permissions.sh && \
python3 <path-to-skill>/scripts/take_screenshot.py --app "<app-name>"
```

For Claude inspection runs, keep the output in temp:
...
```bash
python3 <path-to-skill>/scripts/take_screenshot.py --app "<app-name>" --window-name "Settings"
```

- List matching window ids before capturing (macOS only):

```bash
python3 <path-to-skill>/scripts/take_screenshot.py --list-windows --app "<app-name>"
```
