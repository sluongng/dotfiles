---
name: cli-creator
description: Build a composable CLI for Claude from API docs, an OpenAPI spec, existing curl examples, an SDK, a web app, an admin tool, or a local script. Use when the user wants Claude to create a command-line tool that can run from any repo, expose composable read/write commands, return stable JSON, manage auth, and pair with a companion skill.
---

# CLI Creator

Create a real CLI that future Claude Code sessions can run by command name from any working directory.
...
- Default to **Rust** for a durable CLI Claude should run from any repo: one fast binary, strong argument parsing, good JSON handling, easy copy/install into `~/.local/bin`.
...
Do not expose only a generic `request` command. Give Claude high-level verbs for the repeated jobs.
...
Write the companion skill in the order a future Claude Code session should use the CLI, not as a tour of every feature. Explain:
...
Keep API reference details in the CLI docs or a skill reference file. Keep the skill focused on ordering, safety, and examples future Claude Code sessions should actually run.
